...
class Perfil extends Service
{
	...
	public function _main(Request $request)
	{
		...
		if ($profile->eyes == "AVELLANA")
			...
			// get the eye tone
		$eyesTone = "";
		if ($profile->eyes == "NEGRO" || $profile->eyes == "CARMELITA" ||
				 $profile->eyes == "AVELLANA")
			$eyesTone = "oscuros";
	...
	public function _editar (Request $request)
	{
		...
		$di = \Phalcon\DI\FactoryDefault::getDefault();
		$wwwroot = $di->get('path')['root'];
		$image = empty($person->thumbnail) ? array() : array($person->thumbnail);
	 ...
	 * @return Response/void
	 */
	private function subserviceEnum(Request $request, $field, array $enum, $wrong_subject, $prefix = null, $synonymous = array())
	{
		if ( ! is_null($prefix))
	...
	private function subserviceSimple (Request $request, $field, $prefix = null)
	{
		...
		if (! empty($value)) $this->update("$field = '$value'", $request->email);
		else $this->update("$field = null", $request->email);
